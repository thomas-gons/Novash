...
void prompt_symbols_init() {
  ...
  PS1_blocks[0] = (PS1_block_t){.text = "\\u@\\h",
                                .leading = NULL,
                                .trailing = TRAILING_DIAMOND,
                                .color = {.fg = NULL, .bg = BG_COLOR_BLUE}};
  PS1_blocks[1] = (PS1_block_t){.text = "\\W",
                                .leading = NULL,
                                .trailing = TRAILING_DIAMOND,
                                .color = {.fg = NULL, .bg = BG_COLOR_GREEN}};
...
static char *make_ansi_color(const char *fg, const char *bg) {
  ...
  // \001 and \002 are mandatory to inform readline about non-printable
  // characters
...
static size_t ps1_apply_color(char *dst, size_t max, const char *text,
                              PS1_color_t color, bool reverse) {
  ...
  size_t n =
      (size_t)snprintf(dst, max, "%s%s%s", ansi_color, text, COLOR_RESET);
...
static char *ps1_handle_text(const char *text) {
  ...
  while (*text && pos < sizeof(output) - 1) {
    if (*text == '\\') {
      ...
      switch (*text) {
      case 'u':
        rep = sh_identity->username;
        break;
      case 'h':
        rep = sh_identity->hostname;
        break;
      case 'w':
        rep = sh_identity->cwd;
        break;
      ...
      if (rep) {
        ...
        if (pos + rlen >= sizeof(output) - 1)
          rlen = sizeof(output) - 1 - pos;
...
char *prompt_build_ps1() {
  ...
  for (size_t i = 0; PS1_blocks[i].text != NULL; i++) {
    ...
    if (leading == NULL || !utf8_supported)
      leading = " ";
    else
      should_reverse = true;

    pos += ps1_apply_color(buf + pos, sizeof(buf) - pos, leading, b->color,
                           should_reverse);
    ...
    pos += ps1_apply_color(buf + pos, sizeof(buf) - pos, trailing,
                           trailing_color, should_reverse);
    if (b->trailing != NULL) {
