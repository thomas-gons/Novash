...
static inline char peek_prev(lexer_t *lex) {
  return (lex->pos > 0 && lex->pos <= lex->length) ? lex->input[lex->pos - 1]
                                                   : '\0';
}
...
#define is_expansion_char(c)                                                   \
  ((c) == '$' || (c) == '*' || (c) == '?' || is_glob_char(c))
#define is_special_parameter_char(c)                                           \
  ((c) == '$' || (c) == '?' || (c) == '!' || (c) == '-')
#define is_word_char(c)                                                        \
  (!isspace(c) && !is_meta_char(c) && !is_expansion_char(c))
  ...
  if (is_special_parameter_char(peek(lex))) {
    ...
    if (has_curly) {
      if (peek(lex) == '}')
        ...
        pr_err("lexer: unmatched '{' in variable name\n");
    }
...
static word_part_t lex_next_word_part(lexer_t *lex,
                                      quote_context_e *quote_ctx) {
  char c = peek(lex);
  ...
  if (c == '~' && *quote_ctx == QUOTE_NONE && peek_prev(lex) != '~') {
...
static token_t handle_word_token(lexer_t *lex) {
  ...
  char *raw_value =
      xstrdup_n(raw_start, (size_t)(&lex->input[lex->pos] - raw_start));

  return (token_t){.type = TOK_WORD, .raw_value = raw_value, .parts = parts};
}
...
size_t lexer_token_str(token_t tok, char *buf, size_t buf_sz) {
  ...
  switch (tok.type) {
  case TOK_AND:
    return (size_t)snprintf(buf, buf_sz, "[TOK_AND]: &&\n");
  case TOK_BG:
    return (size_t)snprintf(buf, buf_sz, "[TOK_BG]: &\n");
  case TOK_EOF:
    return (size_t)snprintf(buf, buf_sz, "[TOK_EOF]\n");
  case TOK_OR:
    return (size_t)snprintf(buf, buf_sz, "[TOK_OR]: ||\n");
  case TOK_PIPE:
    return (size_t)snprintf(buf, buf_sz, "[TOK_PIPE]: |\n");
  case TOK_REDIR_APPEND:
    return (size_t)snprintf(buf, buf_sz, "[TOK_REDIR_APPEND]: >>\n");
  case TOK_REDIR_IN:
    return (size_t)snprintf(buf, buf_sz, "[TOK_REDIR_IN]: <\n");
  case TOK_REDIR_OUT:
    return (size_t)snprintf(buf, buf_sz, "[TOK_REDIR_OUT]: >\n");
  case TOK_SEMI:
    return (size_t)snprintf(buf, buf_sz, "[TOK_SEMI]: ;\n");
  default:
    break;
  }
  ...
  if (tok.type == TOK_WORD) {
    ...
    if (tok.parts && arrlen(tok.parts) > 0) {
      for (int i = 0; i < arrlen(tok.parts); i++) {
        ...
        const char *q = (p.quote == QUOTE_SINGLE   ? "'"
                         : p.quote == QUOTE_DOUBLE ? "\""
                                                   : "none");
