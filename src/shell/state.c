...
static void init_environment() {
  ...
  int hist_file_len = snprintf(hist_file_buf, PATH_MAX, "%s/%s",
                               sh_state->identity.cwd, HIST_FILENAME);
...
void shell_state_init() {
  ...
  sh_state->support_utf8 =
      (locale != NULL && (strstr(locale, "UTF-8") || strstr(locale, "utf8")));
...
shell_identity_t *shell_state_get_identity() { return &sh_state->identity; }

shell_jobs_t *shell_state_get_jobs() { return &sh_state->jobs; }

shell_last_exec_t *shell_state_get_last_exec() { return &sh_state->last_exec; }
...
char *shell_state_get_flags(void) {
  ...
  if (sh->flags.interactive)
    buf[pos++] = 'i';
  if (sh->flags.job_control)
    buf[pos++] = 'm';
  if (sh->flags.history_enabled)
    buf[pos++] = 'h';
  if (sh->flags.debug)
    buf[pos++] = 'd';
...
bool shell_is_utf8_supported() { return sh_state->support_utf8; }
